	...
	function qp_required_misc(){
		...
		if($(parentLink).length){
			$('a.nav-parent').on('click', function(e){
				...
				// WHen this is clicked, look into the li parent siblings
				// and close all open menu items
				// Then look in the parent of the li (the ul) and look for
				// all open menu items and close them too with bootstrap close function


				// clickedLink.closest('li').siblings('li.nav-item').find('ul.nav').collapse('hide');
				// clickedLink.closest('ul').siblings('ul.nav.nav-pills').find('ul.nav').collapse('hide');
	...
	function qp_task_manager(){
		...
		if($(placeholder).length){
			...
			var users = new Bloodhound({
				...
				prefetch: 'plugins/typeahead/json/data-sample-users.json' // SAMPLE DATA: You will have to pass data from your database instead
			});
			users.initialize();
